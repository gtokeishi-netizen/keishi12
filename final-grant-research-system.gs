/**
 * @fileoverview 【最終完成版】助成金AIリサーチシステム v2.0
 * 
 * 改善点:
 * - カテゴリ・都道府県は名称管理（IDではなく名称で入力）
 * - 採択率は数字のみ（例: 30、85、15）
 * - 31列完全対応（A-AE）
 * - プロンプト精度大幅向上
 * - エラーハンドリング完全強化
...
\`\`\`json
{
  ...
  "deadline_date": "YYYY-MM-DD形式または随時受付なら空文字",
  ...
  try {
    if (prefectureText === ALL_PREFECTURES) {
      ...
      const prefectures = prefectureText.split(',')
        .map(p => p.trim())
        .filter(p => prefectureList.includes(p));
...
function writeAdvancedResultsToSheet(sheet, row, data) {
  try {
    ...
    const columnMappings = [
      { col: 'C', field: 'content_detail' },       // 詳細コンテンツ
      { col: 'D', field: 'excerpt' },              // 概要
      { col: 'E', field: 'post_status' },          // 投稿ステータス
      { col: 'G', field: 'grant_category' },       // 助成金カテゴリ
      { col: 'H', field: 'grant_amount_text' },    // 助成金額（テキスト）
      { col: 'I', field: 'grant_amount_value' },   // 助成金額（数値）
      { col: 'J', field: 'deadline_text' },        // 申請期限（テキスト）
      { col: 'K', field: 'deadline_date' },        // 申請期限（日付）
      { col: 'L', field: 'organization_name' },    // 実施組織名
      { col: 'M', field: 'organization_type' },    // 組織タイプ
      { col: 'N', field: 'target' },               // 対象者・事業
      { col: 'O', field: 'contact' },              // 連絡先
      { col: 'P', field: 'official_url' },         // 公式URL
      { col: 'Q', field: 'prefecture_name' },      // 都道府県名
      { col: 'R', field: 'grant_municipality' },   // 対象市町村
      { col: 'S', field: 'regional_limitation' },  // 地域制限
      { col: 'T', field: 'application_status' },   // 申請状況
      { col: 'U', field: 'application_method' },   // 申請方法
      { col: 'V', field: 'post_tag' },             // タグ
      { col: 'X', field: 'required_documents' },   // 必要書類
      { col: 'Y', field: 'acceptance_rate' },      // 採択率
      { col: 'Z', field: 'difficulty' },           // 難易度
      { col: 'AA', field: 'eligible_expenses' },   // 対象経費
      { col: 'AB', field: 'subsidy_rate' }         // 補助率
    ];
...
// === 初期化完了 ===
Logger.log('AI助成金リサーチシステム v2.0 (最終版) 初期化完了');